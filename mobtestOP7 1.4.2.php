  ...
  */
  ...
  class MobTest implements Plugin
  {
    ...
    public function init()
    {
      ...
      $this->api->schedule(200, array($this,"spawnAllNpcs"), array(), true); //change to set mob spawn delay in ticks (seconds x 20)
      $this->api->schedule(200, array($this,"spawnNightMobs"), array(), true); //LEAVE THIS ONE ALONE, it checks for night time, but will only spawn them once per ingame day
     // $this->spawnAllNpcs();
    ...
    public function spawnAllNpcs()
    {
     ...
     // $npcplayer->spawned    = true;
      ...
      $entityLimit = 100; //SET THIS, AND ALSO THE NIGHTTIME MOB VARIABLE.
      if (($entitiesBefore <= 100) and (($this->api->time->getPhase())=="day")) {
        for ($i=1; $i<=count($this->api->player->online()); $i++) {
          ...
          $entityit = $this->api->entity->add($this->api->level->getDefault(), ENTITY_MOB, $type, array(
            "x"      => $randomAreaX+rand(-5,5),
            "y"      => 80,
            "z"      => $randomAreaZ+rand(-5,5),
            "Health" => 10,
          ));
      ...
      elseif (($this->api->time->getPhase())!="day") {
        console("Passive mobs not spawned: not daytime.");
      }
      else{
        console("Entity limit reached! Passive mobs not spawned");
      }
      $entitiesNow=count($this->api->entity->getAll());
      console("New Mobs: ".($entitiesNow - $entitiesBefore).", Current Entity Count: ".count($this->api->entity->getAll()).", Day Entity Limit: ".$entityLimit.", Players online: ".count($this->api->player->online()));
    }
    ...
    public function spawnNightMobs()
    {
     ...
     // $npcplayer->spawned    = true;
      ...
      if ((($this->api->time->getPhase())=="night") and ($alreadySpawned==false)) {
        ...
        $entitiesNow=count($this->api->entity->getAll());
        console("New Mobs: ".($entitiesNow - $entitiesBefore).", Current Entity Count: ".count($this->api->entity->getAll()).", Night Entity Limit: ".$entityLimit.", Players online: ".count($this->api->player->online()));
      }
